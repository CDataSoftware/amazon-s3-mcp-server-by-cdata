...
CData's Model Context Protocol (MCP) Server for Amazon S3

:heavy_exclamation_mark: This project builds a read-only MCP server. For full read, write, update, delete, and action capabilities and a simplified setup, check out our free [CData MCP Server for Amazon S3 (beta)](https://www.cdata.com/download/download.aspx?sku=SXZK-V&type=beta). 
## Purpose
We created this read-only MCP Server to allow LLMs (like Claude Desktop) to query live data Amazon S3 supported by the [CData JDBC Driver for Amazon S3](https://www.cdata.com/drivers/amazons3/jdbc).

CData JDBC Driver connects to Amazon S3 by exposing them as relational SQL models.

This server wraps that driver and makes Amazon S3 data available through a simple MCP interface, so LLMs can retrieve live information by asking natural language questions — no SQL required.
...
2. Download and install the CData JDBC Driver for {source}: [https://www.cdata.com/drivers/amazons3/download/jdbc](https://www.cdata.com/drivers/amazons3/download/jdbc)
3. License the CData JDBC Driver:
    ...
    * Run the command `java -jar cdata.jdbc.amazons3.jar --license`
    * Enter your name, email, and "TRIAL" (or your license key).
4. Configure your connection to the data source (Salesforce as an example):
    * Run the command `java -jar cdata.jdbc.amazons3.jar` to open the Connection String utility.
...
5. Create a `.prp` file for your JDBC connection (e.g. `amazon-s3.prp`) using the following properties and format:
    * **Prefix** - a prefix to be used for the tools exposed
    * **ServerName** - a name for your server
    * **ServerVersion** - a version for your server
    * **DriverPath** - the full path to the JAR file for your JDBC driver
    * **DriverClass** - the name of the JDBC Driver Class (e.g. cdata.jdbc.amazons3.AmazonS3Driver)
    * **JdbcUrl** - the JDBC connection string to use with the CData JDBC Driver to connect to your data (copied from above)
    * **Tables** - leave blank to access all data, otherwise you can explicitly declare the tables you wish to create access for
      ```env
      Prefix=amazons3
      ServerName=CDataAmazonS3
      ServerVersion=1.0
      DriverPath=PATH\TO\cdata.jdbc.amazons3.jar
      DriverClass=cdata.jdbc.amazons3.AmazonS3Driver
      JdbcUrl=jdbc:amazons3:InitiateOAuth=GETANDREFRESH;
      Tables=
      ```
...
```json
{
    ...
    "method": "tools/call",
    "params": {
        "name": "amazon_s3_get_tables",
        "arguments": {}
    }
}
```

#### amazon_s3_get_columns
```json
{
    "jsonrpc": "2.0",
    "id": 2,
